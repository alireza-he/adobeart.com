...
const subcategories = {
    'تصویر': ['منظره', 'هنری', 'پرتره', 'ماکرو'],
    'ویدئو': ['تبلیغاتی', 'تیزر', 'مستند', 'فیلمبرداری هوایی'],
    'وب سایت': ['طراحی', 'سئو', 'لوگو', 'پوستر'],
};
...
const Filters = ({onFilterChange, onSearchChange}) => {
    ...
    const handleFilterChange = (type, value) => {
        const newFilters = {
            ...
            subcategory: type === 'category' ? '' : filters.subcategory,
        };
    ...
    const handleSearchChange = (event) => {
        const newSearch = {...search, term: event.target.value};
        setSearch(newSearch);
        onSearchChange(newSearch.term, newSearch.category);
    };

    const handleSearchCategoryChange = (event) => {
        const newSearch = {...search, category: event.target.value};
        setSearch(newSearch);
        onSearchChange(newSearch.term, newSearch.category);
    };
                    ...
                    <Grid container spacing={1}>
                        {['تصویر', 'ویدئو', 'وب سایت'].map((category) => (
                            <Grid item key={category}>
                                <Button
                                    className={filters.category === category ? styles.activeButton : ""}
                ...
                <Grid container spacing={1}>
                    {['اشتراک', 'رایگان'].map((payment) => (
                        <Grid item key={payment}>
                            <Button
                                className={filters.payment === payment ? styles.activeButton : ""}
                                onClick={() => handleFilterChange('payment', payment)}
                            >
                                {payment}
                                {payment !== "اشتراک" ? IconSubscription : IconFree}
                            </Button>
                        </Grid>
                    ))}
                </Grid>
                    ...
                    <Grid container spacing={1}>
                        {['پربازدیدتر', 'جدیدتر', 'قدیمی تر'].map((popularity) => (
                            <Grid item key={popularity}>
                                <Button
                                    className={filters.popularity === popularity ? styles.activeButton : ""}
